...
import { OrderItemType, PaymentFormType, PaymentResultSchemaType, ProductItemType, ShippingSchemaType } from "@/types";
  ...
  if (activeDeal.data) {
    ...
    if (data && data?.stock < 5) {
      await sendMessageToSocket({
        channel: COMMON_CONSTANTS.PUBLISH_KEYS.INVENTORY_UPDATE,
