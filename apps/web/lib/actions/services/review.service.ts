...
export async function handleAddOrEditReview(data: AddReviewSchemaType) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    const avgRating = await tx.review.aggregate({ _avg: { rating: true }, where: { productId: review.productId } });
    const reviewCount = await tx.review.count({ where: { productId: review.productId } });

    await tx.product.update({
      where: { id: review.productId },
      data: { rating: avgRating._avg.rating || 0, numReviews: reviewCount },
    });
  });
...
export async function handleHasUserReviewByProduct(productId: string) {
  const session = await auth();
  if (!session) throw new Error("Not authenticated");
  const data = await prisma.review.findFirst({ where: { productId, userId: session.user.id } });
...
export async function handleHasPurchaseHistory(productId: string) {
  ...
  const hasPreviousOrder = await prisma.order.findFirst({
    where: { userId: session.user.id, orderItems: { some: { productId: productId } } },
  });
