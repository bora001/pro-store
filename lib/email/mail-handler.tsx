...
export const sendPurchaseReceipt = async ({
  ...
  const html = await render(<PurchaseReceiptTemplate order={order} />, {
    pretty: true,
  });
...
export const sendWelcomeEmail = async ({
  ...
  const html = await render(<WelcomeMailTemplate name={name} />, {
    pretty: true,
  });
  return await sendEmail({
    html,
    to: email,
    subject: "Welcome to Pro-store!",
  });
};
...
export const sendEmailVerification = async ({
  ...
  const html = await render(<EmailVerificationTemplate token={token} />, {
    pretty: true,
  });
  return await sendEmail({
    html,
...
export const sendDeleteAccountConfirm = async ({
  email,
  name,
}: {
  email: string;
  ...
  const html = await render(<DeleteAccountTemplate name={name} />, {
    pretty: true,
  });
  return await sendEmail({
    html,
    to: email,
    subject: "Your Pro-Store Account Deletion Request Has Been Processed.",
  });
};
